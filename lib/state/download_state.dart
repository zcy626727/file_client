...
import '../service/file/download_service.dart';
...
class DownloadState extends ChangeNotifier {
  ...
  void initDownloadTask() async {
    ...
    for (var task in totalTaskList) {
      if (task.status == DownloadTaskStatus.finished.index) {
        finishedTaskList.add(task);
      } else if (task.status == DownloadTaskStatus.error.index) {
        errorTaskList.add(task);
      } else {
        task.status = DownloadTaskStatus.pause.index;
  ...
  void addDownloadTask(DownloadTask task) async {
    ...
    if (downloadingTaskList.length > FileConfig.downloadParallelNumber) {
      ...
      log("加到入上传队列");
      task.status = DownloadTaskStatus.downloading.index;
  ...
  void removeDownloadTask(DownloadTask task) async {
    if (task.status == DownloadTaskStatus.downloading.index) {
      //移除并杀死
      downloadingTaskList.remove(task);
      var isolate = downloadIsolateMap.remove(task);
      assert(isolate != null, "上传文件的isolate一定存在于uploadingIsolateMap中");
      isolate!.kill();
    } else if (task.status == DownloadTaskStatus.awaiting.index) {
      awaitingTaskList.remove(task);
    } else if (task.status == DownloadTaskStatus.pause.index) {
      pauseTaskList.remove(task);
    } else if (task.status == DownloadTaskStatus.error.index) {
      errorTaskList.remove(task);
    } else if (task.status == DownloadTaskStatus.finished.index) {
  ...
  Future<void> doDownload(DownloadTask task) async {
    ...
    receivePort.listen(
      (msg) async {
        if (msg is SendPort) {
          ...
          //上传出现异常
          task.status = DownloadTaskStatus.error.index;
          ...
          //上传结束
          task.status = DownloadTaskStatus.finished.index;
          //从上传队列移除
          downloadingTaskList.remove(task);
          //从上传map移除
          downloadIsolateMap.remove(task);
    ...
    var isolate =
        await Isolate.spawn(DownloadService.startIsolate, receivePort.sendPort);
